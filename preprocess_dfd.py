...
def write_tfrecord(df_features, output_file="dfd_features.tfrecord"):
...
# Find all video files in the DFD dataset (both real and fake)
def get_dfd_videos(dfd_path):
    videos = []
    # Get fake videos
    fake_path = os.path.join(os.path.expanduser(dfd_path), 'manipulated')
    ...
    # Get real videos
    real_path = os.path.join(os.path.expanduser(dfd_path), 'original')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process DFD dataset videos and extract features')
    parser.add_argument('--dfd_path', type=str, default='~/balanced_dataset', help='Path to DFD dataset')
    parser.add_argument('--output_dir', type=str, default='data_dfd', help='Directory to save extracted frames')
    parser.add_argument('--output_tfrecord', type=str, default='dfd_features.tfrecord', help='Output TFRecord file path')
    parser.add_argument('--max_workers', type=int, default=1, help='Maximum number of threads to use')
    args = parser.parse_args()
    
    # Update global paths
    global DFD_PATH, OUTPUT_DIR
    ...
    # Get video files
    video_list = get_dfd_videos(DFD_PATH)
    print(f"Found {len(video_list)} videos in the DFD dataset")
    
    # Process videos
    data = []
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = [executor.submit(process_video, video_info) for video_info in video_list]
