...
# Path constants - will be updated from command line arguments
EVAL_DIR = 'eval'
...
def write_tfrecord(df_features, output_file="eval_features.tfrecord"):
...
def process_video(video_path, label, output_frames_dir):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process evaluation dataset videos and extract features')
    parser.add_argument('--eval_dir', type=str, default='eval', help='Path to evaluation dataset with real/fake subdirectories')
    parser.add_argument('--output_dir', type=str, default='eval_data', help='Directory to save extracted frames')
    parser.add_argument('--output_tfrecord', type=str, default='eval_features.tfrecord', help='Output TFRecord file path')
    ...
    # Create lists to store video paths and their labels
    ...
    if not os.path.exists(fake_dir) or not os.path.exists(real_dir):
        print(f"Error: Evaluation dataset must contain 'fake' and 'real' subdirectories at {EVAL_DIR}")
        return
    
    # Get video files from both directories
    fake_videos = [(os.path.join(fake_dir, f), 1, os.path.join(OUTPUT_DIR, 'fake', os.path.splitext(f)[0])) 
                  for f in os.listdir(fake_dir) if f.lower().endswith('.mp4')]
    
    real_videos = [(os.path.join(real_dir, f), 0, os.path.join(OUTPUT_DIR, 'real', os.path.splitext(f)[0])) 
                  for f in os.listdir(real_dir) if f.lower().endswith('.mp4')]
    
    # Make sure the output directories for frames exist
    ...
    if not df_features.empty:
        ...
        print("No videos were successfully processed. Check for errors and try again.")
